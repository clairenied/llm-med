...
export async function PUT(
  ...
  try {
    ...
    if (existingInvitation.usedAt) {
      return NextResponse.json({ error: 'Cannot edit used invitation' }, { status: 400 });
    }
    ...
    if (email !== existingInvitation.email) {
      ...
      if (invitationWithEmail && invitationWithEmail.id !== id) {
        return NextResponse.json({ error: 'Invitation with this email already exists' }, { status: 400 });
      }
    }
