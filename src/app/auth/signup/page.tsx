...
function SignUpForm() {
  ...
  useEffect(() => {
    ...
    if (!invitationId) {
      setError(
        "Account creation requires an invitation. Please contact an administrator.",
      );
      return;
    }
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900 dark:text-white">
            {invitationInfo
              ? `Join as ${invitationInfo.role.toLowerCase()}`
              : "Invitation Required"}
          </h2>
          {invitationInfo ? (
